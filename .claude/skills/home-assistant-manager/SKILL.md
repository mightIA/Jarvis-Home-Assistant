...
> **Source amont** : https://github.com/komal-SkyNET/claude-skill-homeassistant (MIT)
> **Adapté pour Mickael** : HA local `192.168.1.11:2096`, distant `ha.might.ovh`. Remplacer `homeassistant.local` par l'IP locale dans toutes les commandes SSH.
...
1. SSH access to Home Assistant instance (`root@192.168.1.11`)
2. `hass-cli` installed locally (`pipx install homeassistant-cli`)
...
ssh root@192.168.1.11 "ha core check"

# Restart Home Assistant
ssh root@192.168.1.11 "ha core restart"

# View logs
ssh root@192.168.1.11 "ha core logs"

# Tail logs with grep
ssh root@192.168.1.11 "ha core logs | grep -i error | tail -20"
```
...
ssh root@192.168.1.11 "cd /config && git pull"

# 5. Reload or restart
hass-cli service call automation.reload  # if reload sufficient
# OR
ssh root@192.168.1.11 "ha core restart"  # if restart needed
...
# 1. Make changes locally
# 2. Quick deploy
scp automations.yaml root@192.168.1.11:/config/
...
```bash
sleep 3
ssh root@192.168.1.11 "ha core logs | grep -i 'automation_name' | tail -20"
```
...
**For notifications:** ask user if they received it, check logs for mobile_app messages
**For device control:** `hass-cli state get switch.device_name`
**For sensors:** `hass-cli state get sensor.new_sensor`
...
scp .storage/lovelace.control_center root@192.168.1.11:/config/.storage/
...
scp .storage/lovelace.new_dashboard root@192.168.1.11:/config/.storage/
scp .storage/lovelace_dashboards root@192.168.1.11:/config/.storage/
...
**Use Panel View when:** displaying full-screen map (vacuum, cameras), single large card needs full width, want zero margins/padding, minimize scrolling

**Use Sections View when:** organizing multiple cards, need responsive grid layout, building multi-section dashboards
...
**Counting Open Doors:**
```jinja2
{% set door_sensors = ['binary_sensor.front_door', 'binary_sensor.back_door'] %}
...
**Problem 1: Dashboard Not in Sidebar** — Cause: file created but not registered. Fix: add to `.storage/lovelace_dashboards` and restart HA.

**Problem 2: "Configuration Error" in Card** — Cause: custom card not installed, wrong syntax, template error. Fix: check HACS, browser console (F12), test in Dev Tools → Template.

**Problem 3: Auto-Entities Fails** — Cause: `card_param` not supported. Fix: use cards that accept `entities` (entities, vertical-stack, horizontal-stack work; grid, glance don't).

**Problem 4: Vacuum Map Has Margins/Scrolling** — Cause: sections view has margins. Fix: use panel view.

**Problem 5: Template Type Errors** — `TypeError: '<' not supported between str and int`. Fix: `states('sensor.days') | int < 7`.

### Dashboard Debugging

1. **Browser Console (F12):** check for red errors. Common: "Custom element doesn't exist" → card not installed.
2. **Validate JSON Syntax:** `python3 -m json.tool .storage/lovelace.control_center > /dev/null`
3. **Test Templates:** HA → Developer Tools → Template
4. **Verify Entities:** `hass-cli state get binary_sensor.front_door`
5. **Clear Browser Cache:** Ctrl+F5, or incognito window
...
# Logs
ssh root@192.168.1.11 "ha core logs | tail -50"
ssh root@192.168.1.11 "ha core logs | grep -i error | tail -20"
...
ssh root@192.168.1.11 "cd /config && git pull"
scp file.yaml root@192.168.1.11:/config/

# Dashboard deployment
scp .storage/lovelace.my_dashboard root@192.168.1.11:/config/.storage/
...
hass-cli service call automation.reload
hass-cli service call automation.trigger --arguments entity_id=automation.name
ssh root@192.168.1.11 "ha core logs | grep -i 'automation' | tail -10"
```
