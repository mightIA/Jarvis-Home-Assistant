...
last_update: 2026-05-05 (S112 — T#94 sous-objectif 1bis path-token cleanup acquis)
version: 8.1 (post-S112)
---
...
|---|---|---|
| Sessions enregistrées | 112 (S1→S112) | 05/05/2026 |
| Première session | 12/04/2026 | — |
| Dernière session | S112 — T#94 sous-objectif 1bis path-token cleanup ACQUIS : `/private_xxx` → `/jarvis-mcp` (HA UI add-on + `.mcp.json` Cowork + `~/.hermes/config.yaml` Hermès patchés, `hermes mcp test ha-mcp` ✓ Connected 951ms 83 outils, curl 405/404 client+serveur). Diag pré-cleanup : path-token = security-through-obscurity (HTTP 403 sans Service Tokens). Mystère orthogonal : `claude mcp list` Failed depuis S109, 3 skills CLI tournent quand même → mini-tâche orthogonale créée. 3 pièges capitalisés (Get-Content -Raw -Encoding UTF8 explicite, wsl bash -lc login shell, Select-String -SimpleMatch désactive regex). T#94 reste in_progress (2a+2b futurs). | 05/05/2026 |
...
- **S112** (05/05) — **T#94 sous-objectif 1bis path-token cleanup ACQUIS** : `/private_6G36IXICr8K4HJv02SXU9OlE` → `/jarvis-mcp` (non-secret stable). HA UI add-on patché + Stop/Start, `.mcp.json` Cowork L29 patché, `~/.hermes/config.yaml` Hermès L373 patché perms 600 préservées, taille -22 bytes delta exact. Tests bout-en-bout : curl NEW=405 ✅ + OLD=404 ✅ confirmés client+serveur (logs add-on `172.30.33.3:46820`), `hermes mcp test ha-mcp` ✓ Connected 951ms 83 outils ✅, `claude mcp list` URL MAJ + statut Failed inchangé (statu quo bug `${env:VAR}`). Diag pré-cleanup : curl sans Service Tokens → HTTP 403 → CF Access fait l'auth, path-token = security-through-obscurity, suppression non-dégradante sécu. Mystère orthogonal : `claude mcp list` Failed depuis S109 mais 3 skills CLI (`check-jarvis-alert`, `rapport-journalier-reactif`, `ha-logs-archive`) tournent (T#34 logs S100) → mini-tâche orthogonale créée non bloquante. **3 pièges capitalisés** : (P1) `Get-Content -Raw` sans `-Encoding UTF8` explicite → double-encodage non-ASCII régression cosmétique L35 `.mcp.json` ; (P2) `wsl bash -c` shell non-interactif sans PATH user → `wsl bash -lc` login shell pour Hermès ; (P3) `Select-String -SimpleMatch` cherche littéralement le pattern OR `a|b|c`. **Livrables** : récit S112 + 3 auto-memories projet + 4 fichiers patchés (`Cloudflare_Access_Ha-mcp.md` L48, `task_094.md` frontmatter+section+livrable, `.mcp.json` L29+L37, `~/.hermes/config.yaml` L373) + 2 backups conservés. T#94 reste `in_progress` (1bis acquis, 2a IP allowlist + 2b rate limiting reportés). **34 ouvertes / 64 archivées**.
