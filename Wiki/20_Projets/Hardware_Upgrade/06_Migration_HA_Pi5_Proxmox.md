---
title: 06 — Migration HA Pi5 vers VM Proxmox
created: 2026-04-27
migrated_from: Projets/Hardware_Upgrade/06_Migration_HA_Pi5_Proxmox.md
status: en-attente-phase-A-Hermes
phase: 0
budget_target: 2410 EUR
bloqueur: hermes-phase-1bis-d
tags: [projet, hardware, proxmox, ryzen, ha-os, migration]
---
...
curl -I https://mcp.might.ovh/private_<REDACTED>
# Sortie attendue : HTTP/2 405 Method Not Allowed (normal, GET non supporté)
```

> Note : la valeur réelle de `private_<REDACTED>` est conservée dans `.mcp.json` local (non versionné). Voir auto-memories `feedback_secret_path_s48_jamais_applique.md` et `reference_ha_mcp_secret_regeneration.md` pour le contexte de la régénération non appliquée S48 (deux secret_path coexistent en mémoire, seul l'historique S17 est l'actif réel ; la rotation S48 n'a jamais été poussée côté add-on).
...
- Document copié depuis `Projets/Hardware_Upgrade/06_Migration_HA_Pi5_Proxmox.md` (S55 + S59).
- **Patterns sensibles redactés** : 2 occurrences de `private_<REDACTED>` (secret_path ha-mcp) en section 8.3. Source intacte. Voir T#11 pour décision Mickael sur la rotation effective du secret.
